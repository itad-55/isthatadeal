...
def build_email_html(deals, period, show_verify=False, paid=False):
    ...
    teaser_row = '' if paid else (
        '<tr><td style="padding:6px 16px 10px;text-align:center;border-bottom:1px solid #E8E6DF">'
        f'<div style="font-size:13px;color:#8A8680;font-family:monospace;">Paid subscribers are seeing 10 more deals this week. <a href="{STRIPE_LINK}" style="color:#0A7A3E;font-weight:600;text-decoration:none;">Upgrade here →</a></div>'
        '</td></tr>'
    )
    ...
    upgrade_block = '' if paid else (
        ...
        '⭐ Get 10 more deals like this for $2.99/month'
        '</div>'
        '<div style="font-size:15px;color:#0D0D0D;line-height:1.6">'
        'Paid subscribers get all 20 deals every Thursday. One extra deal acted on pays for months of subscription. It also helps keep the project alive.'
        '</div>'
    ...
    midlist_cta = '' if paid else (
        '<tr><td style="padding:10px 18px;text-align:center;border-top:1px solid #E8E6DF;border-bottom:1px solid #E8E6DF;background:#FAFAF7">'
        f'<div style="font-size:13px;color:#4A4740;">Enjoying this? <a href="{STRIPE_LINK}" style="color:#0A7A3E;font-weight:600;text-decoration:none;">Get 10 more deals for $2.99/month →</a></div>'
        '</td></tr>'
    )
    ...
    html = html.replace('{{TEASER_ROW}}', teaser_row)
